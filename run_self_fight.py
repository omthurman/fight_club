...
def create_env():
    ...
    if os.path.isfile(MAIN_MODEL):
        opponent_ppo = PPO.load(MAIN_MODEL)

        # Create a new graph for opponent_ppo
        opponent_ppo.graph = torch.Generator().get_state()[0]

        # Create assign ops for the opponent
        update_ops = []
        update_placeholders = []
        for param in opponent_ppo.policy.parameters():
            placeholder = torch.zeros(param.shape, dtype=param.dtype, requires_grad=True)

            update_ops.append(param.data.copy_(placeholder.detach()))
    ...
    def player2_reset():
        ...
        if random.random() < 0.01:
            ...
            if rand < 0.2:
                ...
                opponent_ppo.load(load_file)
            else:
                # Load most recent params
                print("---Loading new params---")
                opponent_ppo.load(MAIN_MODEL)
                opponent_ppo.use_random_agent = False
...
def run_experiment(args):
    ...
    policy_kwargs = dict(net_arch=dict(pi=[256, 256, 256, 256, 256], vf=[256, 256, 256, 256, 256], activation_fn=nn.ELU))
    ...
    if args.load_model is None:
        ...
        model = PPO.load(args.load_model, env=vecEnv)
